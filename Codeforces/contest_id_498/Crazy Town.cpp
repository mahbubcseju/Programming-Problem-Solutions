...
int main()
{
    ...
    LL sum1=0;
    line p;
    p=line(point((double)x,(double)y),point((double)x1,(double)y1));
    FL(t,1,tc)
    {
        LL a,b,c;
        SL3(a,b,c);
        line q=line((double)a,(double)b,(double)c);
        point mx=q.crosspoint(p);
        if(mx.x>=min(x,x1)&&mx.x<=max(x,x1)&&mx.y>=min(y,y1)&&mx.y<=max(y,y1))sum1++;
    }
    PL(sum1);
    return 0;
}
