...
P query_x(int nx,int sx,int se,int qx1,int qx2,int qy1,int qy2) {
    if(sx>=qx1&&se<=qx2) {
        // cout<<sx<<se<<endl;
        return query_y(nx,sx,se,1,1,m,qy1,qy2);
    }
    int mid=(sx+se)>>1;
...
int main()
{
   ...
   // lol;
    ...
    fr(i,1,n)
    {
      ...
      {
          if(a[i][j]>-1)cnt++;
          cum[i][j]=cum[i-1][j]+cnt;

          if(i>=k&&j>=k)
          {
              int lo=cum[i][j]-cum[i-k][j]-cum[i][j-k]+cum[i-k][j-k];
              if(lo==k*k)
              {
                  //cerr<<i<<" "<<j<<" "<<lo<<" "<<k<<endl;
                  P q=query_x(1,1,n,i-k+1,i,j-k+1,j);
                  ans=min(ans,q.x);
