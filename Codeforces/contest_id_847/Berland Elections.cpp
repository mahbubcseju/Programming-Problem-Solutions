...
struct data
{
    ...
    {
        return ( vote>p.vote || ( vote==p.vote && last<p.last) );
    }
...
//            rep(j,1,n)
//            {
//                TTTT( j, ar[j].id , ar[j].vote, ar[j].last );
//            }
//        }
//
//        pline;
    ...
    rep(i,1,n)
    {
        ...
        while(baki && id<=n)
        {
            ...
            while(baki && br[id].vote<=br[pos].vote)
            {
                br[id].vote++;
                br[id].last=++tt;
                baki--;
